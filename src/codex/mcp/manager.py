...
class McpServerInstance:
    ...
    def __init__(self, name: str, description: str, command: str, args: list[str], env: Optional[dict] = None):
        self.name = name
...
class McpManager:
    ...
    async def start_all(self) -> dict[str, bool]:
        ...
        for name, server in self.servers.items():
            ...
            if success:
                print(f"[green]✅ {name}: 加载了 {len(server.mcp_tools)} 个工具[/green]")
            else:
                print(f"[red]❌ {name}: 启动失败[/red]")
        return results
