...
### Bridge Server 架构（Chrome 扩展桥接 - 推荐用于 RJCut Studio）

```
┌─────────────┐     ┌───────────────────┐     ┌─────────────────────┐
│  Codex CLI  │────▶│  Bridge Server    │────▶│  Chrome Extension   │
│  (codex.py) │ WS  │  (ws:8766)        │ HTTP│  (MCP Proxy)        │
└─────────────┘     └───────────────────┘     └─────────────────────┘
                                                   │
                                                   ▼
                                      ┌─────────────────────────┐
...
1. **加载 Chrome 扩展**
   ...
   - 启用开发者模式
   - 加载 `studio/mcp-proxy-extension` 目录

2. **启动 Bridge Server**
   ```bash
   cd studio/mcp-bridge-server
   npm install
   npm start
   ```
...
3. **配置 Codex**
   ...
   - 启用 `chrome_vfs` 服务器
   - 设置 `websocket_url: "ws://localhost:8766/mcp"`
